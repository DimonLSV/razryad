...
(function () {
  ...
  function renderHistory() {
    ...
    $$('[data-history-backplot]').forEach(b => b.onclick = () => { const x = historyLoad().find(i => i.id === +b.dataset.historyBackplot); if (!x || !x.gcode) return toast('В записи нет G-кода'); window.RazryadBackplot.open(x.gcode,{title:`O${x.programNo||'----'} · история`,source:'История программ'}); });
  }
