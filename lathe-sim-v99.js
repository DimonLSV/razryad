...
function defaults(){return{dialect:'haas',operation:'external',stock:'solid',contour:'step',stockD:60,length:120,boreD:20,targetD:45,stepD:54,stepLen:42,chuck:'3jaw',grip:25,tool:'cnmg',nose:0.8,depth:2,feed:0.25,rpm:800,speed:2,coolant:true,chips:true,arcCenterDiameter:false,showCycles:true,toolConfigs:{}};}
...
function parseGcode(code,rawCfg){
 const cfg={...defaults(),arcCenterDiameter:false,showCycles:true,...(rawCfg||{})},p=profile(),issues=[],segments=[],seen=new Set();
 ...
 const micron=(value,u=unit,word,r,addr)=>{const v=Math.abs(Number(value)||0);if(!v)return 0;
  if(fanuc)return(word&&word.hasDecimal?v:v/1000)*u;
  /* Ветка v>50 — это разрыв в 1000× между соседними целыми: Q50 читается как 50 мм,
     а Q51 как 0.051 мм. Молча угадывать масштаб нельзя, поэтому там, где эвристика
     реально переворачивает трактовку, говорим об этом вслух и показываем оба чтения. */
  if(r&&word&&!word.hasDecimal&&v>50)add('warn',`${addr||word.key}${v} без десятичной точки прочитано как ${(v/1000).toFixed(3)} мм (формат микрон). На Haas глубина задаётся в миллиметрах — если имелось ${v} мм, поставьте точку.`,r.line);
  return(word&&word.hasDecimal?v:v>50?v/1000:v)*u;};
 const wordOf=(r,key)=>r&&r.all&&r.all.find(w=>w.key===key)||null;
 /* Циклы клевка ограничены счётчиком проходов, чтобы описка в шаге не подвесила разбор.
    Раньше выход по счётчику был неотличим от нормального завершения: цикл обрывался
    на середине отверстия, отвод рисовался из недостигнутой точки, и ни ошибки, ни
    предупреждения не возникало. Теперь обрыв всегда называет себя. */
 const guardTrip=new Set();
 const guardHit=(r,cycle,limit)=>{const key=cycle+':'+r.line;
  if(!guardTrip.has(key)){guardTrip.add(key);
   add('warn',`${cycle}: расчёт прерван на ${limit} проходах — цикл не дошёл до заданной глубины, а отвод показан из недостигнутой точки. Проверьте шаг клевка и глубину.`,r.line);}
  return false;};
...
const spec=segment.toolSpec||stationSpec(segment.toolStation),op=segment.operation,freeLen=Math.max(1,cfg.length-cfg.grip),axial=['drill','centerdrill','tap'].includes(op),samples=segmentSamples(segment);if(segment.compMode&&!(spec.nose>0))add('bad','G41/G42 требует задать радиус вершины для активного инструмента.',segment.line,segment);
  ...
  for(const q of samples){const r=Math.abs(q.x)/2,k=Math.max(0,Math.min(collisionMat.z.length-1,Math.round((q.z+cfg.length)/cfg.length*(collisionMat.z.length-1)))),outer=collisionMat.outer[k],inner=collisionMat.inner[k];
   ...
   else if(op==='boring'){if(spec.minBore&&inner*2+0.05<spec.minBore&&q.z<=0)add('bad',`Расточная оправка требует отверстие не меньше Ø${spec.minBore} мм.`,segment.line,segment);
    if(segment.rapid&&q.z<-.02&&q.z>=-freeLen){const e=envHit(segment,q,true);
     if(e===true){add('bad','Быстрый ход расточного резца пересекает текущую стенку отверстия.',segment.line,segment);break;}
     if(e===null){add('warn','Столкновение в этом кадре не проверено: силуэт инструмента не построен. Сверьте траекторию вручную.',segment.line,segment);break;}}}
   else{if(segment.rapid&&q.z<-.02&&q.z>=-freeLen){const e=envHit(segment,q,false);
     if(e===true){add('bad','Быстрый ход G00 пересекает текущую поверхность заготовки.',segment.line,segment);break;}
     if(e===null){add('warn','Столкновение в этом кадре не проверено: силуэт инструмента не построен. Сверьте траекторию вручную.',segment.line,segment);break;}}}
  }
  /* Съём за проход. Прежде столкновение искалось только на быстрых ходах, поэтому
     рабочий кадр с радиальной глубиной 20 мм проходил без единого замечания.
     Каталог не хранит паспортную глубину резания токарной пластины, поэтому судить
     «слишком глубоко» приложение не вправе — оно называет фактическое врезание и
     просит сверить с каталогом. Порог грубый и масштабируется с деталью: пятикратная
     заданная глубина прохода, но не меньше 10 мм и не меньше 5 % диаметра заготовки. */
  if(!segment.rapid&&segment.cutting&&!axial&&op!=='groove'&&op!=='thread'){
   const dz=Math.abs(segment.to.z-segment.from.z),dx=Math.abs(segment.to.x-segment.from.x)/2;
   if(dz>.5&&dz>=dx){
    let ap=0,last=collisionMat.z.length-1;
    for(const q of samples){
     const rr=Math.abs(q.x)/2,k=Math.max(0,Math.min(last,Math.round((q.z+cfg.length)/cfg.length*last)));
     const cut=op==='boring'?rr-collisionMat.inner[k]:collisionMat.outer[k]-rr;
     if(Number.isFinite(cut)&&cut>ap)ap=cut;
    }
    const limit=spec.maxDepth>0?spec.maxDepth:Math.max((cfg.depth||2)*5,10,(cfg.stockD||60)*.05);
    if(ap>limit+.05)add('warn',`Съём за один проход ${ap.toFixed(1)} мм на радиус — это заметно больше обычного для заданной глубины ${(cfg.depth||2)} мм. Сверьте допустимую глубину резания с каталогом пластины.`,segment.line,segment);
   }
  }
 ...
 records.forEach(r=>{
  ...
  if(gs.includes(72)&&!Number.isFinite(out.P)&&!Number.isFinite(out.Q)){
   if(Number.isFinite(out.W))g72Depth=micron(out.W,unit,wordOf(r,'W'),r,'W');
   if(Number.isFinite(out.D))g72Depth=micron(out.D,unit,wordOf(r,'D'),r,'D');
   if(Number.isFinite(out.R))g72Retract=micron(out.R,unit,wordOf(r,'R'),r,'R');
  ...
  if(gs.includes(72)&&Number.isFinite(out.P)&&Number.isFinite(out.Q)){
   ...
   const depth=Number.isFinite(out.D)?micron(out.D,unit,wordOf(r,'D'),r,'D'):(g72Depth||cfg.depth),step=Math.max(.05,depth);
