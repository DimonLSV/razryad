...
function bindWork(){
 ...
 if($('#qaRun')){$('#qaRun').onclick=analyzeProgram;$('#qaDemo').onclick=()=>{$('#qaCode').value='%\nO0100\nG21 G18 G40 G80 G99\nT0101\nG50 S2500\nG96 S120 M03\nG00 X52 Z2\nG71 P100 Q130 U0.3 W0.1 D2 F0.25\nN100 G00 G42 X30\nN110 G01 Z0\nN120 G01 Z-40\nN130 G01 X45\nG70 P100 Q130\nG40 G00 X80 Z20\nG28 U0 W0\nM30\n%';analyzeProgram();};$('#qaFileBtn').onclick=()=>$('#qaFile').click();$('#qaFile').onchange=async e=>{const f=e.target.files[0];if(f){$('#qaCode').value=await f.text();analyzeProgram();}};$('#qaBackplot').onclick=()=>{const code=$('#qaCode').value.trim();if(!code)return toast('Вставьте или откройте G-код');window.RazryadBackplot.open(code,{title:'Проверка готовой программы',source:'Работа → Проверка G-кода'});};}
